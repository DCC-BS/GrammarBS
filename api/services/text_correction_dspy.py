...
class SegmentInfo(dspy.Signature):
    ...
    explanation: str = dspy.OutputField(desc="A short explanation of the correction use the language of the original text")
...
class TextCorrectionService:
    ...
    def correct_text(self, text: str, options: TextCorrectionOptions) -> Either[str, CorrectionResult]:
        ...
        return right(CorrectionResult(blocks=list(map(lambda segment: CorrectionBlock(original=segment.original,
                corrected=[segment.corrected],
                explanation=segment.explanation), response.segments)),
            original=text))
